...
from embed_and_store import ingest
...
def node_save_results(state: CatalogState) -> CatalogState:
    ...
    embedding_json_path = "catalog_for_embedding.json"
    ...
    print(f"Embedding-ready data saved to {embedding_json_path}")

    state["embedding_json_path"] = embedding_json_path
    return state


def node_embed_and_store(state: CatalogState) -> CatalogState:
    """Embed normalized items and store them in construction.items_catalog."""
    embedding_json_path = state.get("embedding_json_path")
    if not embedding_json_path:
        print("node_embed_and_store: no embedding_json_path in state, skipping.")
        return state

    print(f"Starting embedding & storing from {embedding_json_path}…")
    try:
        ingest(embedding_json_path)
    except Exception as e:
...
def build_catalog_extractor():
    ...
    graph.add_edge("save_results", "embed_and_store")
